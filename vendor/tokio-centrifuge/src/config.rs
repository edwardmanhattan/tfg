...
pub struct Config {
	pub token: String,
	/// Raw bytes for the connect command's `data` field (tfg patch:
	/// Minos reads the API token from `data.token`, not `token`).
...
impl Config {
	...
	/// tfg patch: payload for the connect command's `data` field.
	pub fn with_connect_data(mut self, data: Vec<u8>) -> Self {

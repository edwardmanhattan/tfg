                        ...
                        Ok(Ok(reply)) => {
                            match reply {
                                ...
                                Reply::Error(err) => {
                                    ...
                                    // Surface the refusal code (101/103/107/109)
                                    // through the disconnect callback BEFORE
                                    // closing: the bare Err(()) the connect
                                    // future resolves with cannot express which
                                    // one, and callers route sign-out vs silent
                                    // refresh off exactly this. The connect
                                    // waiters drain here too — the cycle below
                                    // early-returns on the Disconnected state
                                    // without draining them.
                                    {
                                        let mut inner = client3.lock().unwrap();
                                        if inner.state != State::Disconnected {
                                            inner.move_to_disconnected(err.code, &err.message);
                                        }
...
impl Client {
    pub fn new(url: &str, config: Config) -> Self {
        ...
        Self(Arc::new(Mutex::new(ClientInner {
            ...
            token: config.token,
            connect_data: config.connect_data,
            name: config.name,
    ...
    /// tfg patch: replace the connect command's `data` payload (used on
    /// the next handshake, same as `set_token`).
    pub fn set_connect_data(&self, data: Vec<u8>) {
        self.0.lock().unwrap().connect_data = data;
    }
}

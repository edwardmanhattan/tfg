...
fn build_local(
    ...
    let cmake_build_dir = dest.join("build");
    let tile_spec_dir = cmake_build_dir.join("vendor").join("maplibre-tile-spec").join("cpp");
    println!("cargo:rustc-link-search=native={}", cmake_build_dir.display());
    println!("cargo:rustc-link-search=native={}", tile_spec_dir.display());

    // Visual Studio is a multi-config generator: native libraries are emitted
    // below the selected configuration directory (for example, Release), not
    // directly in the CMake build directory. Keep the generic paths above for
    // single-config generators and add the Windows configuration paths so the
    // final Rust link can find the generated MapLibre libraries.
    if target_os == "windows" {
        for configuration in ["Debug", "Release"] {
            let configuration_dir = cmake_build_dir.join(configuration);
            println!("cargo:rustc-link-search=native={}", configuration_dir.display());
            println!(
                "cargo:rustc-link-search=native={}",
                tile_spec_dir.join(configuration).display()
            );
        }
    }
